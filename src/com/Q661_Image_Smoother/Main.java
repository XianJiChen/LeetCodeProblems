package com.Q661_Image_Smoother;
...
class Solution {
    ...
    private boolean isValidCell(int x, int y) {
        return (x >= 0 && x < img.length) && (y >= 0 && y < this.img[0].length);
    }

    private int getAvg(int x, int y, int[][] img) {
        int cnt = 0;
        int ttl = 0;
    ...
    public int[][] imageSmoother(int[][] img) {
        this.img = new int[img.length][img[0].length];
        for (int i = 0; i < img.length; i++) {
            for (int j = 0; j < img[0].length; j++) {
                this.img[i][j] = getAvg(i, j, img);
            }
        }
        return this.img;
    }
...
public class Main {
    public static void main(String[] args) {
        int[][][] inputs = {{{1, 1, 1}, {1, 0, 1}, {1, 1, 1}}, {{100, 200, 100}, {200, 50, 200}, {100, 200, 100}}};

        Solution solution = new Solution();
        for (int[][] input : inputs) {
            for (int[] row : solution.imageSmoother(input)) {
